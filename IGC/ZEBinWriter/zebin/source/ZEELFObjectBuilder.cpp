...
class ELFWriter {
public:
  ELFWriter(llvm::raw_pwrite_stream &OS, ZEELFObjectBuilder &objBuilder);
...
private:
  ...
  SectionHdrEntry &createSectionHdrEntry(const std::string &name, unsigned type, unsigned flags = 0,
  ...
  void writeSymbol(uint32_t name, uint64_t value, uint64_t size, uint8_t binding, uint8_t type, uint8_t other,
                   uint16_t shndx);
  ...
  void writeSecHdrEntry(uint32_t name, uint32_t type, uint64_t flags, uint64_t address, uint64_t offset, uint64_t size,
...
ZEELFObjectBuilder::Section &ZEELFObjectBuilder::addStandardSection(const std::string &sectName, const uint8_t *data,
                                                                    uint64_t size, unsigned type, unsigned flags,
                                                                    uint32_t padding, uint32_t align,
                                                                    StandardSectionListTy &sections) {
  ...
  uint32_t need_padding_for_align = (align == 0) ? 0 : align - ((size + padding) % align);
  if (need_padding_for_align == align)
    need_padding_for_align = 0;

  // total required padding is (padding + need_padding_for_align)
  sections.emplace_back(ZEELFObjectBuilder::StandardSection(sectName, data, size, type, flags,
                                                            (need_padding_for_align + padding), m_sectionIdCount));
  ++m_sectionIdCount;
  return sections.back();
}

ZEELFObjectBuilder::SectionID ZEELFObjectBuilder::addSectionText(const std::string &name, const uint8_t *data,
                                                                 uint64_t size, uint32_t padding, uint32_t align) {
  ...
  Section &sect = addStandardSection(sectName, data, size, ELF::SHT_PROGBITS, ELF::SHF_ALLOC | ELF::SHF_EXECINSTR,
                                     padding, align, m_textSections);

  return sect.id();
}

ZEELFObjectBuilder::SectionID ZEELFObjectBuilder::addSectionData(const std::string &name, const uint8_t *data,
                                                                 uint64_t size, uint32_t padding, uint32_t align,
                                                                 bool rodata, bool alloc) {
  ...
  Section &sect =
      addStandardSection(sectName, data, size, ELF::SHT_PROGBITS, flags, padding, align, m_dataAndbssSections);
  return sect.id();
}

ZEELFObjectBuilder::SectionID ZEELFObjectBuilder::addSectionBss(const std::string &name, uint64_t size,
                                                                uint32_t padding, uint32_t align) {
  ...
  Section &sect = addStandardSection(sectName, nullptr, size, ELF::SHT_NOBITS, ELF::SHF_ALLOC | ELF::SHF_WRITE, padding,
                                     align, m_dataAndbssSections);
  return sect.id();
}

void ZEELFObjectBuilder::addSectionGTPinInfo(const std::string &name, const uint8_t *data, uint64_t size) {
  ...
  addStandardSection(sectName, data, size, SHT_ZEBIN_GTPIN_INFO, 0, 0, 0, m_otherStdSections);
}

void ZEELFObjectBuilder::addSectionVISAAsm(const std::string &name, const uint8_t *data, uint64_t size) {
  // adjust the section name
  std::string sectName;
  if (name != "")
  ...
  addStandardSection(sectName, data, size, SHT_ZEBIN_VISAASM, 0, 0, 0, m_otherStdSections);
}

void ZEELFObjectBuilder::addSectionMisc(const std::string &name, const uint8_t *data, uint64_t size) {
  // adjust the section name
  std::string sectName;
  if (name != "")
  ...
  addStandardSection(sectName, data, size, SHT_ZEBIN_MISC, 0, 0, 0, m_otherStdSections);
}

void ZEELFObjectBuilder::addSectionMetrics(const std::string &name, const uint8_t *data, uint64_t size) {
  // adjust the section name
  std::string sectName;
  if (name != "")
  ...
  addStandardSection(sectName, data, size, ELF::SHT_NOTE, 0, 0, 0, m_otherStdSections);
}

void ZEELFObjectBuilder::addSectionSpirv(const std::string &name, const uint8_t *data, uint64_t size) {
...
ZEELFObjectBuilder::SectionID ZEELFObjectBuilder::addSectionDebug(const std::string &name, const uint8_t *data,
                                                                  uint64_t size) {
  const std::string &sectName = name.empty() ? m_DebugName : name;
  Section &sect = addStandardSection(sectName, data, size, ELF::SHT_PROGBITS, 0, 0, 0, m_otherStdSections);
  return sect.id();
}

void ZEELFObjectBuilder::addSectionZEInfo(zeInfoContainer &zeInfo) {
...
void ZEELFObjectBuilder::addSymbol(const std::string &name, uint64_t addr, uint64_t size, uint8_t binding, uint8_t type,
                                   ZEELFObjectBuilder::SectionID sectionId) {
  if (binding == llvm::ELF::STB_LOCAL)
    m_localSymbols.emplace_back(ZEELFObjectBuilder::Symbol(name, addr, size, binding, type, sectionId));
  else
    m_globalSymbols.emplace_back(ZEELFObjectBuilder::Symbol(name, addr, size, binding, type, sectionId));
}

ZEELFObjectBuilder::RelocSection &ZEELFObjectBuilder::getOrCreateRelocSection(SectionID targetSectId,
                                                                              bool isRelFormat) {
  ...
  for (RelocSectionListTy::reverse_iterator it = m_relocSections.rbegin(); it != m_relocSections.rend(); ++it) {
...
void ZEELFObjectBuilder::addRelRelocation(uint64_t offset, const std::string &symName, R_TYPE_ZEBIN type,
                                          SectionID sectionId) {
  RelocSection &reloc_sect = getOrCreateRelocSection(sectionId, true);
  // create the relocation
  reloc_sect.m_Relocations.emplace_back(ZEELFObjectBuilder::Relocation(offset, symName, type));
}

void ZEELFObjectBuilder::addRelaRelocation(uint64_t offset, const std::string &symName, R_TYPE_ZEBIN type,
                                           uint64_t addend, SectionID sectionId) {
  RelocSection &reloc_sect = getOrCreateRelocSection(sectionId, false);
  // create the relocation
  reloc_sect.m_Relocations.emplace_back(ZEELFObjectBuilder::Relocation(offset, symName, type, addend));
}
...
ZEELFObjectBuilder::SectionID ZEELFObjectBuilder::getSectionIDBySectionName(const char *name) {
...
void ELFWriter::writePadding(uint32_t size) { m_W.OS.write_zeros(size); }
...
void ELFWriter::writeSymbol(uint32_t name, uint64_t value, uint64_t size, uint8_t binding, uint8_t type, uint8_t other,
                            uint16_t shndx) {
...
uint64_t ELFWriter::writeRelocTab(const RelocationListTy &relocs, bool isRelFormat) {
  ...
  for (const ZEELFObjectBuilder::Relocation &reloc : relocs) {
    ...
    if (isRelFormat)
      writeRelRelocation(reloc.offset(), reloc.type(), m_SymNameIdxMap[reloc.symName()]);
    else
      writeRelaRelocation(reloc.offset(), reloc.type(), m_SymNameIdxMap[reloc.symName()], reloc.addend());
  }
...
uint64_t ELFWriter::writeSymTab() {
  ...
  auto writeOneSym = [&](ZEELFObjectBuilder::Symbol &sym) {
    ...
    writeSymbol(nameoff, sym.addr(), sym.size(), sym.binding(), sym.type(), 0, sect_idx);
...
std::pair<uint64_t, uint64_t> ELFWriter::writeCompatibilityNote() {
  ...
  writeOneNote("IntelGT", static_cast<uint32_t>(m_ObjBuilder.m_productFamily), NT_INTELGT_PRODUCT_FAMILY);

  // write NT_INTELGT_GFXCORE_FAMILY_
  writeOneNote("IntelGT", static_cast<uint32_t>(m_ObjBuilder.m_gfxCoreFamily), NT_INTELGT_GFXCORE_FAMILY);

  // write NT_INTELGT_TARGET_METADATA
  writeOneNote("IntelGT", m_ObjBuilder.m_metadata.packed, NT_INTELGT_TARGET_METADATA);

  // write NT_INTELGT_ZEBIN_VERSION
  writeOneStrNote("IntelGT", PreDefinedAttrGetter::getVersionNumber(), NT_INTELGT_ZEBIN_VERSION);
  ...
  if (m_ObjBuilder.m_zeInfoSection &&
      std::any_of(m_ObjBuilder.m_zeInfoSection->getZeInfo().kernels.begin(),
                  ...
                  [](const zeInfoKernel &kernel) { return kernel.execution_env.has_stack_calls; })) {
    // writeNT_INTELGT_VISA_ABI_VERSION
    writeOneNote("IntelGT", m_ObjBuilder.getVISAABIVersion(), NT_INTELGT_VISA_ABI_VERSION);
  }

  // write NT_INTELGT_PRODUCT_CONFIG
  writeOneNote("IntelGT", m_ObjBuilder.m_gmdID.Value, NT_INTELGT_PRODUCT_CONFIG);

  // write NT_INTELGT_INDIRECT_ACCESS_DETECTION_VERSION
  writeOneNote("IntelGT", INDIRECT_ACCESS_DETECTION_VERSION, NT_INTELGT_INDIRECT_ACCESS_DETECTION_VERSION);

  // write NT_INTELGT_INDIRECT_ACCESS_BUFFER_MAJOR_VERSION
  writeOneNote("IntelGT", INDIRECT_ACCESS_BUFFER_MAJOR_VERSION, NT_INTELGT_INDIRECT_ACCESS_BUFFER_MAJOR_VERSION);
...
void ELFWriter::writeSecHdrEntry(uint32_t name, uint32_t type, uint64_t flags, uint64_t address, uint64_t offset,
...
void ELFWriter::writeSectionHeader() {
  ...
  for (SectionHdrEntry &entry : m_SectionHdrEntries) {
    writeSecHdrEntry(entry.name, entry.type, entry.flags, 0, entry.offset, entry.size, entry.link, entry.info, 0,
                     entry.entsize);
  }
}
...
void ELFWriter::writeSections() {
  for (SectionHdrEntry &entry : m_SectionHdrEntries) {
    ...
    case SHT_ZEBIN_GTPIN_INFO: {
      ...
      [[maybe_unused]] auto res = symName.consume_front(m_ObjBuilder.m_GTPinInfoName);
      IGC_ASSERT(res);
    ...
    case SHT_ZEBIN_MISC: {
      ...
      const StandardSection *const stdsect = static_cast<const StandardSection *>(entry.section);
    ...
    case ELF::SHT_RELA: {
      ...
      const RelocSection *const relocSec = static_cast<const RelocSection *>(entry.section);
    ...
    case ELF::SHT_NULL:
      // the first entry
      entry.size = (m_SectionHdrEntries.size() + 1) >= ELF::SHN_LORESERVE ? (m_SectionHdrEntries.size() + 1) : 0;
      break;
...
void ELFWriter::writeSectionHdrOffset(uint64_t offset) {
  ...
  if (is64Bit()) {
    uint64_t Val = support::endian::byte_swap<uint64_t>(offset, m_W.Endian);
    stream.pwrite(reinterpret_cast<char *>(&Val), sizeof(Val), offsetof(ELF::Elf64_Ehdr, e_shoff));

  } else {
    uint32_t Val = support::endian::byte_swap<uint32_t>(static_cast<uint32_t>(offset), m_W.Endian);
    stream.pwrite(reinterpret_cast<char *>(&Val), sizeof(Val), offsetof(ELF::Elf32_Ehdr, e_shoff));
  }
}
...
void ELFWriter::writeHeader() {
  ...
  m_W.write<uint16_t>(is64Bit() ? sizeof(ELF::Elf64_Ehdr) : sizeof(ELF::Elf32_Ehdr));

  m_W.write<uint16_t>(0); // e_phentsize = prog header entry size
  m_W.write<uint16_t>(0); // e_phnum = # prog header entries = 0

  // e_shentsize
  m_W.write<uint16_t>(is64Bit() ? sizeof(ELF::Elf64_Shdr) : sizeof(ELF::Elf32_Shdr));

  // TODO: We do not support the case that the number of sections is greater
  // than or equal to SHN_LORESERVE now.
  IGC_ASSERT_MESSAGE(numOfSections() < ELF::SHN_LORESERVE, "# of sections should be less than SHN_LORESERVE (oxff00)");
  // e_shnum
...
ELFWriter::SectionHdrEntry &ELFWriter::createSectionHdrEntry(const std::string &name, unsigned type, unsigned flags,
                                                             const Section *sect) {
  SectionHdrEntry &entry = m_SectionHdrEntries.emplace_back(SectionHdrEntry());
  entry.type = type;
  entry.flags = flags;
...
void ELFWriter::createSectionHdrEntries() {
  ...
  // .symtab
  if (!m_ObjBuilder.m_localSymbols.empty() || !m_ObjBuilder.m_globalSymbols.empty()) {
    m_SymTabIndex = index;
    ++index;
  ...
  if (!m_ObjBuilder.m_relocSections.empty()) {
    ...
    for (RelocSection &sect : m_ObjBuilder.m_relocSections) {
      SectionHdrEntry &entry = sect.isRelFormat() ? createSectionHdrEntry(sect.m_sectName, ELF::SHT_REL, 0, &sect)
  ...
  // .ze_info
  if (m_ObjBuilder.m_zeInfoSection) {
    createSectionHdrEntry(m_ObjBuilder.m_ZEInfoName, SHT_ZEBIN_ZEINFO, 0, m_ObjBuilder.m_zeInfoSection.get());
    ++index;
  }
...
bool ZEInfoBuilder::empty() const { return mContainer.kernels.empty(); }

// addPayloadArgumentByPointer - add explicit kernel argument with pointer
// type into given arg_list
zeInfoPayloadArgument &ZEInfoBuilder::addPayloadArgumentByPointer(PayloadArgumentsTy &arg_list, int32_t offset,
  ...
  if (addrmode == PreDefinedAttrGetter::ArgAddrMode::slm && addrspace == PreDefinedAttrGetter::ArgAddrSpace::local) {
...
zeInfoPayloadArgument &ZEInfoBuilder::addPayloadArgumentImage(PayloadArgumentsTy &arg_list, int32_t offset,
                                                              int32_t size, int32_t arg_index,
                                                              PreDefinedAttrGetter::ArgAddrMode addrmode,
                                                              PreDefinedAttrGetter::ArgAccessType access_type,
...
zeInfoPayloadArgument &ZEInfoBuilder::addPayloadArgumentSampler(PayloadArgumentsTy &arg_list, int32_t offset,
                                                                int32_t size, int32_t arg_index, int32_t sampler_index,
                                                                PreDefinedAttrGetter::ArgAddrMode addrmode,
                                                                PreDefinedAttrGetter::ArgAccessType access_type,
...
zeInfoInlineSampler &ZEInfoBuilder::addInlineSampler(InlineSamplersTy &inline_sampler_list, int32_t sampler_index,
...
zeInfoPayloadArgument &ZEInfoBuilder::addPayloadArgumentByValue(PayloadArgumentsTy &arg_list, int32_t offset,
                                                                int32_t size, int32_t arg_index, int32_t source_offset,
                                                                bool is_ptr) {
  ...
  if (!arg_list.empty()) {
    ...
    mergeable = prev.arg_index == arg_index && (prev.source_offset + prev.size == source_offset) &&
                (prev.offset + prev.size == offset) && !prev.is_ptr && !is_ptr;
  }
...
zeInfoPayloadArgument &ZEInfoBuilder::addPayloadArgumentImplicit(PayloadArgumentsTy &arg_list,
                                                                 PreDefinedAttrGetter::ArgType type, int32_t offset,
                                                                 int32_t size, bool has_ptr) {
  zeInfoPayloadArgument &arg = arg_list.emplace_back();
...
// into given zeKernel. The type must be local_size, group_size,
// global_id_offset or private_base_stateless
zeInfoPayloadArgument &ZEInfoBuilder::addPayloadArgumentImplicitInlineSampler(PayloadArgumentsTy &arg_list,
...
zeInfoPerThreadPayloadArgument &ZEInfoBuilder::addPerThreadPayloadArgument(PerThreadPayloadArgumentsTy &arg_list,
                                                                           PreDefinedAttrGetter::ArgType type,
                                                                           int32_t offset, int32_t size) {
...
zeInfoBindingTableIndex &ZEInfoBuilder::addBindingTableIndex(BindingTableIndicesTy &bti_list, int32_t bti_value,
                                                             int32_t arg_index) {
...
zeInfoPerThreadMemoryBuffer &ZEInfoBuilder::addPerThreadMemoryBuffer(PerThreadMemoryBuffersTy &mem_buff_list,
...
zeInfoPerThreadMemoryBuffer &ZEInfoBuilder::addScratchPerThreadMemoryBuffer(PerThreadMemoryBuffersTy &mem_buff_list,
                                                                            PreDefinedAttrGetter::MemBufferUsage usage,
                                                                            int32_t slot_id, int32_t size) {
  zeInfoPerThreadMemoryBuffer &info = mem_buff_list.emplace_back();
...
zeInfoPerThreadMemoryBuffer &
ZEInfoBuilder::addPerSIMTThreadGlobalMemoryBuffer(PerThreadMemoryBuffersTy &mem_buff_list,
                                                  PreDefinedAttrGetter::MemBufferUsage usage, int32_t size) {
  zeInfoPerThreadMemoryBuffer &info = mem_buff_list.emplace_back();
...
void ZEInfoBuilder::addExpPropertiesHasNonKernelArgLdSt(zeInfoKernel &zekernel, bool hasNonKernelArgLoad,
                                                        bool hasNonKernelArgStore, bool hasNonKernelArgAtomic) {
