...
static bool samplesAveragedEqually(const std::vector<Instruction *> &similarSampleInsts) {
  ...
  for (auto sampleInst : similarSampleInsts) {
    ...
    for (int i = 0; i < 3; i++) {
      if (inst->getOpcode() == Instruction::FMul) { //% 29 = fmul fast float %texel_x, 2.500000e-01
        if (!texels.erase(inst->getOperand(0)) && !texels.erase(inst->getOperand(1))) {
          return false;
        }
...
static bool detectSampleAveragePattern2(const std::vector<Instruction *> &sampleInsts, Instruction *texSample) {
  ...
  for (unsigned i = 0; i < nSampleInsts; i++) {
    ...
    for (unsigned j = 0; j < 3; j++) {
      ...
      if (i == 0) {
        ...
        if (fadd == nullptr || fadd->getOpcode() != Instruction::FAdd || fadd->hasNUsesOrMore(2)) {
          return false;
        }
        if (fadd->getOperand(0) != rgb[idx] && fadd->getOperand(1) != rgb[idx]) {
          return false;
        }
        rgb[idx] = fadd;
      }
  ...
  for (unsigned i = 0; i < 3; i++) {
    ...
    if (fadd->getOperand(0) != base[i] && fadd->getOperand(1) != base[i]) {
      return false;
    }

    Instruction *fmul = dyn_cast<Instruction>(*fadd->users().begin());
    if (fmul == nullptr || fmul->getOpcode() != Instruction::FMul || fmul->hasNUsesOrMore(2)) {
      return false;
    }
    ConstantFP *cf;
    ...
    if (cf == nullptr || !cf->getType()->isFloatTy() || cf->getValueAPF().convertToFloat() != averagingFactor) {
      return false;
    }
  }
  return true;
...
// @llvm.genx.GenISA.sampleptr2 => samples(tex1....) ---> This will be texelSample, sampling from tex1. We search
// similar to this
...
bool GatingSimilarSamples::checkAndSaveSimilarSampleInsts() {
  for (auto &I : *BB) {
    if (SampleIntrinsic *SI = dyn_cast<SampleIntrinsic>(&I)) {
      ...
      } else { // we can't have a different texel sample between 2 matching(similar) texel samples!
        return false;
      }
    }
  }
...
bool GatingSimilarSamples::setOrCmpGatingValue(Value *&gatingValueToCmp1, Instruction *mulInst,
                                               const Instruction *texelSampleInst) {
  if (!gatingValueToCmp1) {
    // This is the first texel sample inst from the loop after unrolled
    IGC_ASSERT_MESSAGE(texelSampleInst == similarSampleInsts[0],
                       "incorrect inst sequence while extracting the loop gating value");
...
bool GatingSimilarSamples::findAndSetCommonGatingValue() {
  ...
  for (auto &texelSampleInstInLoop : similarSampleInsts) {
    ...
    if (!(firstOp && secondOp))
      return false;
    if (firstOp->getOpcode() == Instruction::FSub ||
        firstOp->getOpcode() == Instruction::FAdd) { // i.e. (texcoord0 (+/-) something)
      ...
      if (mayBeMulInst->getOpcode() == Instruction::FMul) { // i.e. something is FMul!
        if (!setOrCmpGatingValue(gatingValue_mul1, mayBeMulInst, texelSampleInstInLoop))
          return false;
      } else if (mayBeMulInst->getOpcode() == Instruction::FSub ||
                 mayBeMulInst->getOpcode() == Instruction::FAdd) { // that means we have this "tc - (0 - -FMul)"
...
bool GatingSimilarSamples::areSampleInstructionsSimilar(Instruction *firstSampleInst, Instruction *secondSampleInst) {
  if (!firstSampleInst || !secondSampleInst)
    return false;
...
bool GatingSimilarSamples::runOnFunction(llvm::Function &F) {
  ...
  for (BasicBlock::reverse_iterator rItr = BB->rbegin(); rItr != BB->rend(); rItr++) {
    if (GenIntrinsicInst *GenI = dyn_cast<GenIntrinsicInst>(&*rItr)) {
      ...
      isOutputInstr |= GenI->getIntrinsicID() == GenISAIntrinsic::GenISA_OUTPUTPS;
      if (isOutputInstr) {
        resultInst = &*rItr;
        break;
      }
  ...
  IGCLLVM::TerminatorInst *thenBlockTerminator =
      SplitBlockAndInsertIfThen(isGatingValueNotZero, similarSampleInsts[0], false);
  ...
  IGCLLVM::splice(thenBlock, thenBlock->begin(), tailBlock, similarSampleInsts[0]->getIterator(),
                  resultInst->getIterator());
...
IGC_INITIALIZE_PASS_BEGIN(GatingSimilarSamples, "loop-gating", "Loop Gating Optimization", false, false)
INITIALIZE_PASS_DEPENDENCY(CodeGenContextWrapper)
IGC_INITIALIZE_PASS_END(GatingSimilarSamples, "loop-gating", "Loop Gating Optimization", false, false)

llvm::FunctionPass *IGC::CreateGatingSimilarSamples() { return new GatingSimilarSamples(); }
