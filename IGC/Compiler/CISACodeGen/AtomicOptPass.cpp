...
AtomicOptPass::AtomicOptPass() : FunctionPass(ID) { initializeAtomicOptPassPass(*PassRegistry::getPassRegistry()); }
...
Value *AtomicOptPass::getSubgroupLocalIdBI(Instruction *Pos) {
  ...
  Function *simdLaneIdIntrinsic = GenISAIntrinsic::getDeclaration(Pos->getModule(), GenISAIntrinsic::GenISA_simdLaneId);
...
bool AtomicOptPass::runOnFunction(Function &F) {
  Changed = false;
  M = F.getParent();
  llvm::SmallVector<std::tuple<Instruction *, BasicBlock *, BasicBlock *, Instruction *, size_t>, 32>
      AtomicsEmulationToProcess;
