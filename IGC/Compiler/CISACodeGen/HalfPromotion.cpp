...
HalfPromotion::HalfPromotion() : FunctionPass(ID) { initializeHalfPromotionPass(*PassRegistry::getPassRegistry()); }
...
void IGC::HalfPromotion::handleLLVMIntrinsic(llvm::IntrinsicInst &I) {
  ...
  if (id == Intrinsic::cos || id == Intrinsic::sin || id == Intrinsic::log2 || id == Intrinsic::exp2 ||
      id == Intrinsic::sqrt || id == Intrinsic::floor || id == Intrinsic::ceil || id == Intrinsic::fabs ||
      id == Intrinsic::pow || id == Intrinsic::fma || id == Intrinsic::maxnum || id == Intrinsic::minnum) {
    ...
    Value *f32Val = builder.CreateCall(pNewFunc, arguments);
...
void IGC::HalfPromotion::handleGenIntrinsic(llvm::GenIntrinsicInst &I) {
  ...
  if (id == GenISAIntrinsic::GenISA_WaveAll || id == GenISAIntrinsic::GenISA_WavePrefix ||
      id == GenISAIntrinsic::GenISA_WaveClusteredPrefix || id == GenISAIntrinsic::GenISA_WaveClustered ||
      id == GenISAIntrinsic::GenISA_WaveInterleave || id == GenISAIntrinsic::GenISA_WaveClusteredInterleave) {
    Module *M = I.getParent()->getParent()->getParent();
    llvm::IGCIRBuilder<> builder(&I);
    std::vector<llvm::Value *> arguments;

    Function *pNewFunc = GenISAIntrinsic::getDeclaration(M, I.getIntrinsicID(), builder.getFloatTy());
...
void HalfPromotion::visitBinaryOperator(llvm::BinaryOperator &BI) {
  if (BI.getType()->isHalfTy() && (BI.getOpcode() == BinaryOperator::FAdd || BI.getOpcode() == BinaryOperator::FSub ||
                                   BI.getOpcode() == BinaryOperator::FMul || BI.getOpcode() == BinaryOperator::FDiv)) {
...
void HalfPromotion::visitCastInst(llvm::CastInst &CI) {
  if (CI.getType()->isHalfTy() && (CI.getOpcode() == CastInst::UIToFP || CI.getOpcode() == CastInst::SIToFP)) {
    ...
    m_changed = true;
             ...
             (CI.getOpcode() == CastInst::FPToUI || CI.getOpcode() == CastInst::FPToSI)) {
    llvm::IGCIRBuilder<> builder(&CI);
    Value *newOp = nullptr;
