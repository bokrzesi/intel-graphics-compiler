...
LivenessAnalysis::~LivenessAnalysis() { clear(); }
...
void LivenessAnalysis::initValueIds() {
  int ix = 0;
  for (Function::arg_iterator AI = m_F->arg_begin(), AE = m_F->arg_end(); AI != AE; ++AI) {
    Value *Val = &*AI;
  ...
  for (inst_iterator II = inst_begin(m_F), IE = inst_end(m_F); II != IE; ++II) {
    Instruction *Inst = &*II;
...
void LivenessAnalysis::calculate(Function *F) {
  ...
  for (LiveVars::iterator LVI = m_LV->begin(), LVE = m_LV->end(); LVI != LVE; ++LVI) {
    ...
    for (auto II = lvi->AliveBlocks.begin(), IE = lvi->AliveBlocks.end(); II != IE; ++II) {
      BasicBlock *BB = *II;
      setLiveIn(BB, valID);
    }

    for (std::vector<Instruction *>::iterator II = lvi->Kills.begin(), IE = lvi->Kills.end(); II != IE; ++II) {
      Instruction *inst = *II;
      setKillInsts(V, inst);
...
void LivenessAnalysis::dump() { print(dbgs()); }

void LivenessAnalysis::dump_livein(BasicBlock *BB) { print_livein(dbgs(), BB); }
