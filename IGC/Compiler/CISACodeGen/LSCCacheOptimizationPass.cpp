...
void LSCCacheOptimizationPass::visitStoreInst(StoreInst &storeInst) {
  ...
  if (offset + data_size > 32) {
    ...
    auto *bitcast1 = builder.CreateBitCast(initial_pointer, builder.getPtrTy(addrspace));
  ...
  // load only the green blocks if they are contiguous
    ...
    if (offset == 0) {
      ...
      // %0 = bitcast <>* %baseAddress to i8*
      auto *bitcast1 = builder.CreateBitCast(initial_pointer, builder.getIntPtrTy(DL, addrspace));
