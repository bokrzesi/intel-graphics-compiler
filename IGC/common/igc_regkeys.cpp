...
void DEVICE_INFO::get_device_property(DEVINST deviceInstace, DWORD property) {
  ...
  CONFIGRET status = CM_Get_DevNode_Registry_PropertyW(deviceInstace, property, NULL, NULL, &propertyDataLength, 0);

  // Then fetch the actual property data
  if (CR_BUFFER_SMALL == status) {
    status = CM_Get_DevNode_Registry_PropertyW(deviceInstace, property, NULL, &propertyData[0], &propertyDataLength, 0);
  }

  if (CR_SUCCESS == status) {
    // Convert from UTF-16 (wide char) to UTF-8
    DWORD convertedStringSize =
        WideCharToMultiByte(CP_UTF8, 0, &propertyData[0], sizeof(propertyData), NULL, 0, NULL, NULL);
    std::string propertyString(convertedStringSize, 0);
    WideCharToMultiByte(CP_UTF8, 0, &propertyData[0], sizeof(propertyData), &propertyString[0], convertedStringSize,
                        NULL, NULL);
...
/*                                                                                          */
/********************************************************************************************/
static void ConvertDoubleNullTermStringToVector(std::vector<wchar_t> &input_str,
                                                std::vector<std::wstring> &output_vec) {
...
/*                                                                                           */
/*********************************************************************************************/
static bool GetPropertyFromDevice(DEVINST devInst, const DEVPROPKEY &PropertyKey, std::vector<BYTE> &PropertyData) {
  ...
  do {
    ...
    do {
      ...
      cr = CM_Get_Device_ID_List_SizeW(
          &DeviceIDListSize, DisplayDevClassGUID,
      ...
      cr =
          CM_Get_Device_ID_ListW(DisplayDevClassGUID, DeviceIDList.data(), DeviceIDListSize, CM_GETIDLIST_FILTER_CLASS);
    ...
    for (auto device : Devices) {
      if ((device.find(L"VEN_8086") != std::wstring::npos) || (device.find(L"ven_8086") != std::wstring::npos)) {
        // Found an intel device, add it to the list
        DEVINST devInst;
        CONFIGRET crLocateDevNode =
            CM_Locate_DevNodeW(&devInst, const_cast<wchar_t *>(device.c_str()), CM_LOCATE_DEVNODE_NORMAL);
...
bool ReadIGCRegistry(const char *pName, void *pValue, unsigned int size, bool readFromEnv) {
  ...
  for (const std::string &registrykeypath : registrykeypaths) {
    HKEY uscKey;
    success = RegOpenKeyExA(HKEY_LOCAL_MACHINE, registrykeypath.c_str(), 0, KEY_READ, &uscKey);
...
template <typename T> static std::string ConvertDefault(const std::string &Type, T Val, const std::string &ValStr) {
  if (Type == "bool") {
...
static void DumpIGCRegistryKeyDefinitions(const char *registryKeyPath, const char *xmlPath) {
#ifdef _WIN32
...
void DumpIGCRegistryKeyDefinitions() {
  ...
  constexpr const char *xmlPath = "C:\\Intel\\IGfx\\GfxRegistryManager\\Keys\\IGC.xml";
...
void DumpIGCRegistryKeyDefinitions3(std::string driverRegistryPath, unsigned long pciBus, unsigned long pciDevice,
  ...
  const std::string registryKeyPath = "HKLM\\SYSTEM\\ControlSet001\\Control\\Class\\" + driverRegistryPath + "\\IGC";

  const std::string xmlPath = "C:\\Intel\\IGfx\\GfxRegistryManager\\Keys\\IGC." + std::to_string(pciBus) + "." +
...
static void setRegkeyFromOption(const std::string &optionValue, const char *dataTypeName, const char *regkeyName,
                                void *pRegkeyVar, bool &isKeySet) {
  ...
  bool isInt = (strcmp(dataTypeName, "int") == 0) || (strcmp(dataTypeName, "DWORD") == 0);
...
static std::optional<std::pair<HashRange::Type, llvm::StringRef>> parseHashType(llvm::StringRef line) {
...
static std::optional<std::pair<bool, llvm::StringRef>> detectEntryPoints(llvm::StringRef &line) {
  using namespace llvm;
  size_t Loc = line.find_first_of(':');
  if (Loc == StringRef::npos)
    return {};
...
static void ParseHashRange(llvm::StringRef line, std::vector<HashRange> &ranges) {
  ...
  do {
    ...
    StringRef end = (dash == StringRef::npos) ? start : token.substr(dash + 1);
    HashRange range = {};
...
void SetCurrentDebugHash(const ShaderHash &hash) { g_CurrentShaderHash = hash; }
...
void GetKeysSetExplicitly(std::string *KeyValuePairs, std::string *OptionKeys) {
  ...
  for (DWORD i = 0; i < NUM_REGKEY_ENTRIES; i++) {
    ...
    if (strcmp("ShaderDumpEnableAll", key) == 0 || strcmp("ShaderDumpEnable", key) == 0 ||
        strcmp("ShaderDumpRegexFilter", key) == 0 || strcmp("DumpToCurrentDir", key) == 0 ||
        strcmp("EnableCosDump", key) == 0 || strcmp("DumpToCustomDir", key) == 0 ||
        strcmp("EnableDxbcDump", key) == 0 || strcmp("EnableDxAsmDump", key) == 0) {
      continue;
    }
